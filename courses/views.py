...
def course_detail(request, pk):
    ...
    # chain puts a set of iterables together
    # sorted sorts an iterabled based on a key
    # lambda is an anonymous function, at each iteration it uses the order variable to supply for the sort
...
def quiz_create(request, course_pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            # make model but don't update database
...
def quiz_edit(request, course_pk, quiz_pk):
    ...
    # get instance of quiz needed to be edited
...
# when rendering formset
@login_required
def answer_form(request, question_pk):
    question = get_object_or_404(models.Question, pk=question_pk)

    # shows all answers associated with the given question
    ...
    if request.method == 'POST':
        ...
        if formset.is_valid():
            # answers have no associated question
            answers = formset.save(commit=False)

            for answer in answers:
                # answers linked to question
                answer.question = question
                answer.save()
